...
import (
	...
	goCrypto "github.com/ligo-libs/crypto-go"
...
type PublicLigoChainAPI struct {
	b Backend
}

func NewPublicLigoChainAPI(b Backend) *PublicLigoChainAPI {
	return &PublicLigoChainAPI{b}
}

func (s *PublicLigoChainAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
	price, err := s.b.SuggestPrice(ctx)
	return (*hexutil.Big)(price), err
}

func (s *PublicLigoChainAPI) ProtocolVersion() hexutil.Uint {
	return hexutil.Uint(s.b.ProtocolVersion())
}

func (s *PublicLigoChainAPI) Syncing() (interface{}, error) {
...
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Ligo Chain Signed Message:\n%d%s", len(data), data)
...
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (string, error) {
	...
	if sig[64] != 27 && sig[64] != 28 {
		return "", fmt.Errorf("invalid Ligo Chain signature (V is not 27 or 28)")
	}
	sig[64] -= 27
...
func (args *SendTxArgs) setDefaults(ctx context.Context, b Backend) error {
	...
	var function = ligoAbi.Unknown
	if ligoAbi.IsLigoChainContractAddr(args.To) {
		var input []byte
		if args.Data != nil {
			input = *args.Data
		...
		if len(input) == 0 {
			return errors.New(`ligochain contract without any data provided`)
		}

		var err error
		function, err = ligoAbi.FunctionTypeFromId(input[:4])
		if err != nil {
			return err
		}
	}

	if function == ligoAbi.DepositInSideChain || function == ligoAbi.WithdrawFromMainChain || function == ligoAbi.SaveDataToMainChain {
	...
	} else if !crypto.ValidateLigoAddr(string(args.To[:])) {
...
type PublicLigoAPI struct {
	am        *accounts.Manager
	b         Backend
	nonceLock *AddrLocker
}

func NewPublicLigoAPI(b Backend, nonceLock *AddrLocker) *PublicLigoAPI {
	return &PublicLigoAPI{b.AccountManager(), b, nonceLock}
}

func (s *PublicLigoAPI) SignAddress(from common.Address, consensusPrivateKey hexutil.Bytes) (goCrypto.Signature, error) {
...
func (api *PublicLigoAPI) WithdrawReward(ctx context.Context, from common.Address, delegateAddress common.Address, gasPrice *hexutil.Big) (common.Hash, error) {
	input, err := ligoAbi.ChainABI.Pack(ligoAbi.WithdrawReward.String(), delegateAddress)
	if err != nil {
		return common.Hash{}, err
	}

	defaultGas := ligoAbi.WithdrawReward.RequiredGas()

	args := SendTxArgs{
		From:     from,
		To:       &ligoAbi.ChainContractMagicAddr,
...
func (api *PublicLigoAPI) Delegate(ctx context.Context, from, candidate common.Address, amount *hexutil.Big, gasPrice *hexutil.Big) (common.Hash, error) {

	input, err := ligoAbi.ChainABI.Pack(ligoAbi.Delegate.String(), candidate)
	if err != nil {
		return common.Hash{}, err
	}

	defaultGas := ligoAbi.Delegate.RequiredGas()
...
func (api *PublicLigoAPI) UnDelegate(ctx context.Context, from, candidate common.Address, amount *hexutil.Big, gasPrice *hexutil.Big) (common.Hash, error) {

	input, err := ligoAbi.ChainABI.Pack(ligoAbi.UnDelegate.String(), candidate, (*big.Int)(amount))
	if err != nil {
		return common.Hash{}, err
	}

	defaultGas := ligoAbi.UnDelegate.RequiredGas()
...
func (api *PublicLigoAPI) Register(ctx context.Context, from common.Address, registerAmount *hexutil.Big, pubkey goCrypto.BLSPubKey, signature hexutil.Bytes, commission uint8, gasPrice *hexutil.Big) (common.Hash, error) {

	input, err := ligoAbi.ChainABI.Pack(ligoAbi.Register.String(), pubkey.Bytes(), signature, commission)
	if err != nil {
		return common.Hash{}, err
	}

	defaultGas := ligoAbi.Register.RequiredGas()
...
func (api *PublicLigoAPI) UnRegister(ctx context.Context, from common.Address, gasPrice *hexutil.Big) (common.Hash, error) {

	input, err := ligoAbi.ChainABI.Pack(ligoAbi.UnRegister.String())
	if err != nil {
		return common.Hash{}, err
	}

	defaultGas := ligoAbi.UnRegister.RequiredGas()
...
func (api *PublicLigoAPI) CheckCandidate(ctx context.Context, address common.Address, blockNr rpc.BlockNumber) (map[string]interface{}, error) {
...
func (api *PublicLigoAPI) GetBannedStatus(ctx context.Context, address common.Address, blockNr rpc.BlockNumber) (map[string]interface{}, error) {
	state, _, err := api.b.StateAndHeaderByNumber(ctx, blockNr)
	if state == nil || err != nil {
		return nil, err
	}
...
func (api *PublicLigoAPI) SetCommission(ctx context.Context, from common.Address, commission uint8, gasPrice *hexutil.Big) (common.Hash, error) {
	input, err := ligoAbi.ChainABI.Pack(ligoAbi.SetCommission.String(), commission)
	if err != nil {
		return common.Hash{}, err
	}

	defaultGas := ligoAbi.SetCommission.RequiredGas()
...
func (api *PublicLigoAPI) EditValidator(ctx context.Context, from common.Address, moniker, website string, identity string, details string, gasPrice *hexutil.Big) (common.Hash, error) {
	input, err := ligoAbi.ChainABI.Pack(ligoAbi.EditValidator.String(), moniker, website, identity, details)
	if err != nil {
		return common.Hash{}, err
	}

	defaultGas := ligoAbi.EditValidator.RequiredGas()
...
func (api *PublicLigoAPI) UnBanned(ctx context.Context, from common.Address, gasPrice *hexutil.Big) (common.Hash, error) {
	input, err := ligoAbi.ChainABI.Pack(ligoAbi.UnBanned.String())
	if err != nil {
		return common.Hash{}, err
	}

	defaultGas := ligoAbi.UnBanned.RequiredGas()
...
func init() {
	...
	core.RegisterValidateCb(ligoAbi.WithdrawReward, withdrawRewardValidateCb)
	core.RegisterApplyCb(ligoAbi.WithdrawReward, withdrawRewardApplyCb)

	core.RegisterValidateCb(ligoAbi.Delegate, delegateValidateCb)
	core.RegisterApplyCb(ligoAbi.Delegate, delegateApplyCb)

	core.RegisterValidateCb(ligoAbi.UnDelegate, unDelegateValidateCb)
	core.RegisterApplyCb(ligoAbi.UnDelegate, unDelegateApplyCb)

	core.RegisterValidateCb(ligoAbi.Register, registerValidateCb)
	core.RegisterApplyCb(ligoAbi.Register, registerApplyCb)

	core.RegisterValidateCb(ligoAbi.UnRegister, unRegisterValidateCb)
	core.RegisterApplyCb(ligoAbi.UnRegister, unRegisterApplyCb)

	core.RegisterValidateCb(ligoAbi.SetCommission, setCommisstionValidateCb)
	core.RegisterApplyCb(ligoAbi.SetCommission, setCommisstionApplyCb)

	core.RegisterValidateCb(ligoAbi.EditValidator, editValidatorValidateCb)

	core.RegisterValidateCb(ligoAbi.UnBanned, unBannedValidateCb)
	core.RegisterApplyCb(ligoAbi.UnBanned, unBannedApplyCb)
}
...
func withDrawRewardValidation(from common.Address, tx *types.Transaction, state *state.StateDB, bc *core.BlockChain) (*ligoAbi.WithdrawRewardArgs, error) {

	var args ligoAbi.WithdrawRewardArgs
	data := tx.Data()
	if err := ligoAbi.ChainABI.UnpackMethodInputs(&args, ligoAbi.WithdrawReward.String(), data[4:]); err != nil {
		return nil, err
	}
...
func registerValidation(from common.Address, tx *types.Transaction, state *state.StateDB, bc *core.BlockChain) (*ligoAbi.RegisterArgs, error) {
	...
	var args ligoAbi.RegisterArgs
	data := tx.Data()
	if err := ligoAbi.ChainABI.UnpackMethodInputs(&args, ligoAbi.Register.String(), data[4:]); err != nil {
		return nil, err
	}
	...
	var ep *epoch.Epoch
	if nc, ok := bc.Engine().(consensus.Tendermint); ok {
...
func delegateValidation(from common.Address, tx *types.Transaction, state *state.StateDB, bc *core.BlockChain) (*ligoAbi.DelegateArgs, error) {

	if tx.Value().Sign() == -1 {
		return nil, core.ErrDelegateAmount
	}

	var args ligoAbi.DelegateArgs
	data := tx.Data()
	if err := ligoAbi.ChainABI.UnpackMethodInputs(&args, ligoAbi.Delegate.String(), data[4:]); err != nil {
		return nil, err
	}
...
func unDelegateValidation(from common.Address, tx *types.Transaction, state *state.StateDB, bc *core.BlockChain) (*ligoAbi.UnDelegateArgs, error) {

	var args ligoAbi.UnDelegateArgs
	data := tx.Data()
	if err := ligoAbi.ChainABI.UnpackMethodInputs(&args, ligoAbi.UnDelegate.String(), data[4:]); err != nil {
		return nil, err
	}
...
func setCommissionValidation(from common.Address, tx *types.Transaction, state *state.StateDB, bc *core.BlockChain) (*ligoAbi.SetCommissionArgs, error) {
	if !state.IsCandidate(from) {
		return nil, core.ErrNotCandidate
	}

	var args ligoAbi.SetCommissionArgs
	data := tx.Data()
	if err := ligoAbi.ChainABI.UnpackMethodInputs(&args, ligoAbi.SetCommission.String(), data[4:]); err != nil {
		return nil, err
	}
...
func editValidatorValidateCb(tx *types.Transaction, state *state.StateDB, bc *core.BlockChain) error {
	...
	var args ligoAbi.EditValidatorArgs
	data := tx.Data()
	if err := ligoAbi.ChainABI.UnpackMethodInputs(&args, ligoAbi.EditValidator.String(), data[4:]); err != nil {
		return err
	}
