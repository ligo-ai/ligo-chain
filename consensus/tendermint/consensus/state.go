...
type ConsensusState struct {
	...
	blockFromMiner *ligoTypes.Block
	backend        Backend
...
func (cs *ConsensusState) LoadCommit(height uint64) *types.Commit {
	...
	ncExtra, height := cs.LoadTendermintExtra(height)
...
func (cs *ConsensusState) enterPropose(height uint64, round int) {
	...
	if cs.state.NTCExtra.NeedToSave &&
		(cs.state.NTCExtra.ChainID != params.MainnetChainConfig.LigoChainId && cs.state.NTCExtra.ChainID != params.TestnetChainConfig.LigoChainId) {
...
func (cs *ConsensusState) createProposalBlock() (*types.NCBlock, *types.PartSet) {
	...
	if cs.blockFromMiner != nil {
		...
		var tx3ProofData []*ligoTypes.TX3ProofData
		txs := ligoBlock.Transactions()
		for _, tx := range txs {
			if ligoAbi.IsLigoChainContractAddr(tx.To()) {
				data := tx.Data()
		...
		return types.MakeBlock(cs.Height, cs.state.NTCExtra.ChainID, commit, ligoBlock,
...
func (cs *ConsensusState) finalizeCommit(height uint64) {
	...
	if cs.state.NTCExtra.Height < block.NTCExtra.Height {
		...
		if block.NTCExtra.ChainID != params.MainnetChainConfig.LigoChainId && block.NTCExtra.ChainID != params.TestnetChainConfig.LigoChainId {
			...
			for _, tx := range txs {
				if ligoAbi.IsLigoChainContractAddr(tx.To()) {
					...
					if function == ligoAbi.WithdrawFromSideChain {
...
func (cs *ConsensusState) defaultSetProposal(proposal *types.Proposal) error {
	...
	if proposal.Round == cs.Round {
		...
		if !cs.GetProposer().PubKey.VerifyBytes(types.SignBytes(cs.chainConfig.LigoChainId, proposal), proposal.Signature) {
			return ErrInvalidProposalSignature
		}

	} else {

		if !cs.proposerByRound(proposal.Round).Proposer.PubKey.VerifyBytes(types.SignBytes(cs.chainConfig.LigoChainId, proposal), proposal.Signature) {
			return ErrInvalidProposalSignature
		}
...
func (cs *ConsensusState) saveBlockToMainChain(block *ligoTypes.Block) {

	client := cs.cch.GetClient()
	ctx, _ := context.WithTimeout(context.Background(), 30*time.Second)

	bs := []byte{}
	proofData, err := ligoTypes.NewSideChainProofData(block)
	if err != nil {
...
func (cs *ConsensusState) broadcastTX3ProofDataToMainChain(block *ligoTypes.Block) {
	client := cs.cch.GetClient()
	ctx, _ := context.WithTimeout(context.Background(), 30*time.Second)

	proofData, err := ligoTypes.NewTX3ProofData(block)
	if err != nil {
