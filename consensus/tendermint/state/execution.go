...
func updateLocalEpoch(bc *core.BlockChain, block *ligoTypes.Block) {
	if block.NumberU64() == 0 {
		return
	}

	ncExtra, _ := types.ExtractTendermintExtra(block.Header())
...
func autoStartMining(bc *core.BlockChain, block *ligoTypes.Block) {
	eng := bc.Engine().(consensus.Tendermint)
	currentEpoch := eng.GetEpoch()
