...
func (sb *backend) Protocol() consensus.Protocol {
	...
	sb.logger.Info("Tendermint backend started")
	...
	if sb.chainConfig.LigoChainId == params.MainnetChainConfig.LigoChainId || sb.chainConfig.LigoChainId == params.TestnetChainConfig.LigoChainId {
		protocolName = "ligochain" //we also use "ligochain" if the net is "testnet"
	} else {
		protocolName = "ligochain_" + sb.chainConfig.LigoChainId
	}
...
func (sb *backend) SetBroadcaster(broadcaster consensus.Broadcaster) {
	...
	//sb.logger.Infof("Tendermint SetBroadcaster: %p", broadcaster)
	sb.broadcaster = broadcaster
}

func (sb *backend) GetBroadcaster() consensus.Broadcaster {

	//sb.logger.Infof("Tendermint GetBroadcaster: %p", sb.broadcaster)
