...
func startHTTP(endpoint string, cors []string, vhosts []string, timeouts rpc.HTTPTimeouts) error {
	...
	var err error
	httpListener, httpMux, err = startLigoChainHTTPEndpoint(endpoint, cors, vhosts, timeouts)
	if err != nil {
		return err
	}
...
func startLigoChainHTTPEndpoint(endpoint string, cors []string, vhosts []string, timeouts rpc.HTTPTimeouts) (net.Listener, *http.ServeMux, error) {
	var (
...
func startWS(endpoint string) error {
	...
	var err error
	wsListener, wsMux, err = startLigoChainWSEndpoint(endpoint)
	if err != nil {
		return err
	}
...
func startLigoChainWSEndpoint(endpoint string) (net.Listener, *http.ServeMux, error) {
	var (
		listener net.Listener
		err      error
	)
