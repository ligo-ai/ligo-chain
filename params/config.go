...
type ChainConfig struct {
	LigoChainId string   `json:"ligoChainId"`
...
func NewSideChainConfig(sideChainID string) *ChainConfig {
	...
	digest := crypto.Keccak256([]byte(config.LigoChainId))
...
func (c *ChainConfig) String() string {
	...
	return fmt.Sprintf("{LigoChainId: %s ChainID: %v Homestead: %v  EIP150: %v EIP155: %v EIP158: %v Byzantium: %v Constantinople: %v Engine: %v}",
		c.LigoChainId,
		c.ChainId,
		c.HomesteadBlock,
		c.EIP150Block,
...
func (c *ChainConfig) IsMainChain() bool {
	return c.LigoChainId == MainnetChainConfig.LigoChainId || c.LigoChainId == TestnetChainConfig.LigoChainId
}

func IsMainChain(chainId string) bool {
	return chainId == MainnetChainConfig.LigoChainId || chainId == TestnetChainConfig.LigoChainId
}
