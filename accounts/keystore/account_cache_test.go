...
func TestCacheInitialReload(t *testing.T) {
	...
	for _, account := range accounts {
		fmt.Printf("TestCacheInitialReload account=%v\n", account)
	}
...
func TestCacheFind(t *testing.T) {
	...
	accs := []accounts.Account{
		...
		{
			Address: common.HexToAddress("3Nm7pC9jsTZyLLiTtbX8aRiYWC3HHjW9SU"),
			URL:     accounts.URL{Scheme: KeyStoreScheme, Path: filepath.Join(dir, "d.key")},
		},
	}
	for _, a := range accs {
		cache.add(a)
