...
func NewDatabase(db ligodb.KeyValueStore) ligodb.Database {
	return db
}

// NewMemoryDatabase creates an ephemeral in-memory key-value database without a
// freezer moving immutable chain segments into cold storage.
func NewMemoryDatabase() ligodb.Database {
...
func NewMemoryDatabaseWithCap(size int) ligodb.Database {
	return NewDatabase(memorydb.NewWithCap(size))
}

// NewLevelDBDatabase creates a persistent key-value database without a freezer
// moving immutable chain segments into cold storage.
func NewLevelDBDatabase(file string, cache int, handles int, namespace string) (ligodb.Database, error) {
