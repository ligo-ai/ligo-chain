...
import (
	...
	"github.com/ligo-ai/ligo-chain/utilities/common/mclock"
	"github.com/ligo-ai/ligo-chain/utilities/common/prque"
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	delFn := func(db ligodb.Writer, hash common.Hash, num uint64) {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= ligodb.IdealBatchSize {
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	...
	nc := bc.Engine().(consensus.Tendermint)
	...
	if withinEpochSwitchWindow || bc.cacheConfig.TrieDirtyDisabled || meetFlushBlockInterval {
		...
		if current := block.NumberU64(); current > triesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - ligodb.IdealBatchSize)
			}
