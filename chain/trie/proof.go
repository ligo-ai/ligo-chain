...
func (t *SecureTrie) Prove(key []byte, fromLevel uint, proofDb ligodb.Writer) error {
	return t.trie.Prove(key, fromLevel, proofDb)
}

func VerifyProof(rootHash common.Hash, key []byte, proofDb ligodb.Reader) (value []byte, nodes int, err error) {
	key = keybytesToHex(key)
	wantHash := rootHash
	for i := 0; ; i++ {
