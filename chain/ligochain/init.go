...
import (
	...
	"github.com/ligo-ai/ligo-chain/utilities/common/math"
...
const (
	...
	DefaultAccountPassword = "ligochain"
)
...
func initLigoGenesis(ctx *cli.Context) error {
	log.Info("this is init-ligo-chain")
	args := ctx.Args()
	if len(args) != 1 {
	...
	log.Infof("this is init-ligo-chain chainId %v", chainId)
	log.Info("this is init-ligo-chain" + ctx.GlobalString(utils.DataDirFlag.Name) + "--" + ctx.Args()[0])
	return init_ligo_genesis(utils.GetTendermintConfig(chainId, ctx), balance_str, isMainnet)
}

func init_ligo_genesis(config cfg.Config, balanceStr string, isMainnet bool) error {

	balanceAmounts, err := parseBalaceAmount(balanceStr)
	if err != nil {
		utils.Fatalf("init ligochain failed")
		return err
	}
	...
	var coreGenesis = core.GenesisWrite{
		...
		Mixhash:    common.Hash{},
		Coinbase:   "LigoAIBlockchainsGenesisCoinbase",
	...
	ligoGenesisPath := config.GetString("ligo_genesis_file")

	if err = ioutil.WriteFile(ligoGenesisPath, contents, 0654); err != nil {
		utils.Fatalf("write ligo_genesis_file failed")
		return err
	}
	return nil
}
...
func initCmd(ctx *cli.Context) error {
	...
	ligoGenesisPath := ctx.Args().First()
	fmt.Printf("int genesis path %v\n", ligoGenesisPath)
	if len(ligoGenesisPath) == 0 {
	...
	return init_cmd(ctx, utils.GetTendermintConfig(chainId, ctx), chainId, ligoGenesisPath)
}
...
func InitSideChainCmd(ctx *cli.Context) error {
	...
	for _, chainId := range chainIds {
		...
		sideConfig := utils.GetTendermintConfig(chainId, ctx)

		ethGenesisPath := sideConfig.GetString("ligo_genesis_file")
		if err := ioutil.WriteFile(ethGenesisPath, ethGenesis, 0644); err != nil {
			utils.Fatalf("write ligo_genesis_file failed")
			return err
		}

		init_ligochain(chainId, ethGenesisPath, ctx)
...
func init_cmd(ctx *cli.Context, config cfg.Config, chainId string, ligoGenesisPath string) error {

	init_ligochain(chainId, ligoGenesisPath, ctx)

	init_em_files(config, chainId, ligoGenesisPath, nil)

	return nil
}

func init_ligochain(chainId string, ligoGenesisPath string, ctx *cli.Context) {

	dbPath := filepath.Join(utils.MakeDataDir(ctx), chainId, clientIdentifier, "/chaindata")
	log.Infof("init_ligochain 0 with dbPath: %s", dbPath)

	chainDb, err := rawdb.NewLevelDBDatabase(filepath.Join(utils.MakeDataDir(ctx), chainId, clientIdentifier, "/chaindata"), 0, 0, "ligochain/db/chaindata/")
	if err != nil {
		utils.Fatalf("could not open database: %v", err)
	}
	defer chainDb.Close()

	log.Info("init_ligochain 1")
	genesisFile, err := os.Open(ligoGenesisPath)
	if err != nil {
		utils.Fatalf("failed to read genesis file: %v", err)
	}
	defer genesisFile.Close()

	log.Info("init_ligochain 2")
	block, err := core.WriteGenesisBlock(chainDb, genesisFile)
	if err != nil {
		utils.Fatalf("failed to write genesis block: %v", err)
	}

	log.Info("init_ligochain end")
...
func init_em_files(config cfg.Config, chainId string, genesisPath string, validators []types.GenesisValidator) error {
	...
	if err != nil {
		utils.Fatalf("failed to read ligochain genesis file: %v", err)
		return err
	}
...
func createGenesisDoc(config cfg.Config, chainId string, coreGenesis *core.Genesis, privValidator *types.PrivValidator, validators []types.GenesisValidator) error {
	...
	if _, err := os.Stat(genFile); os.IsNotExist(err) {
		...
		genDoc := types.GenesisDoc{
			ChainID:      chainId,
			Consensus:    types.CONSENSUS_Tendermint,
...
func createPriValidators(config cfg.Config, num int) []*types.PrivValidator {
	...
	for i := 0; i < num; i++ {
		...
		if err != nil {
			utils.Fatalf("Failed to create LigoAI account: %v", err)
		}
