...
type tester struct {
	workspace string
	stack     *node.Node
	ethereum  *ligoprotocol.LigoAI
	console   *Console
	input     *hookedPrompter
	output    *bytes.Buffer
}

func newTester(t *testing.T, confOverride func(*ligoprotocol.Config)) *tester {
	...
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return ligoprotocol.New(ctx, ethConf) }); err != nil {
