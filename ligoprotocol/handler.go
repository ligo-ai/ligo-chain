...
import (
	...
	"github.com/ligo-ai/ligo-chain/ligoprotocol/fetcher"
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	pm.logger.Debug("Removing Ligo Chain peer", "peer", id)
...
func (pm *ProtocolManager) Stop() {
	pm.logger.Info("Stopping protocol")
	...
	pm.logger.Info("Protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Ligo Chain peer connected", "name", p.Name())
	...
	if err := p.Handshake(pm.networkId, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Ligo Chain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Ligo Chain peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Ligo Chain message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case msg.Code == PreImagesMsg:
		...
		if len(preimagesMap) > 0 {
			db, _ := pm.blockchain.StateCache().TrieDB().DiskDB().(ligodb.Database)
